package com.arifpurnama.imageclassficationtensorflow.classifier;
...
public class ImageClassifier {
    ...
    private static final float PROBABILTY_MEAN = 0.0f;
    ...
    public ImageClassifier(Activity activity) throws IOException {
        //loading the mode
        @NonNull MappedByteBuffer classfierModel = FileUtil.loadMappedFile(activity,
                ".flite");
        labels = FileUtil.loadLabels(activity, ".txt");

        tensorClassifier = new Interpreter(classfierModel, null);
        ...
        //Create Buffer needed for recognetion
        ...
        probabilityProcessor = new TensorProcessor.Builder().add(new NormalizeOp(PROBABILTY_MEAN, PROBABILITY_STD))
                .build();
    }

    public List<Recognition> recognitionImage(final Bitmap bitmap, final int sensorOrientation){
        List<Recognition> recognitions = new ArrayList<>();
        inputImageBuffer = loadImage(bitmap, sensorOrientation);
        tensorClassifier.run(inputImageBuffer.getBuffer(), probabilityImageBuffer.getBuffer().rewind());
        Map<String, Float> labelProbability = new TensorLabel(labels, probabilityProcessor.process(probabilityImageBuffer)).getMapWithFloatValue();
    ...
    public class Recognition implements Comparable {
        ...
        public Recognition(String name, float confident) {
            this.name = name;
        ...
        public int compareTo(Object o) {
            return Float.compare(((Recognition)o).confident, this.confident);
        }
    }
